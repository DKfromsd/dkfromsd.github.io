"use strict"

/*
 * Anthony note: Changing these texts may cause severe side effects. 
 * Please beware of changing the length of texts and number of words. 
 */
const phrase1 = `Nice Graphic Solutions`;
// const phrase2 = `Secure Your SmallBiz`;
const phrase3 = `Dream your Business`;
const phrase4 = `Draw your future`;
const phrase5 = `Display clearly`;
const phrase6 = `Demonstrate Portfolio`;

const email = `mickeyfromsd@gmail.com`;
const phone = `(123) 456-7890`;

const messageSuccess = `Your request is successfully sent.`;
const messageFail = `Sorry my mailbox is not set up yet. Please email us at info@rysonconsulting.com`;

const mainDesc = `
    We offer a personalized Grpafic Design Portfolio at a minimal cost, providing everything you need to 
    jumpstart your illustration skills. With our service, you can present your portfolio with a custom education 
    for your careers within a couple of months.
`;